...
let rec add file =
  try
    ...
    else if (Sys.file_exists file)&&(isMp3 file)&& not(checkExist file 0) then
      begin
    ...
    else if (Sys.file_exists file ) && (isM3u file) then
      begin
...
let connectSort () =
  ...
  let sort_by_nmb (model:#GTree.model) row1 row2 =
    let name1 = model#get ~row: row1 ~column: UiPage1.nmb in
    let name2 = model#get ~row: row2 ~column: UiPage1.nmb in
    compare name1 name2 in

  let sort_by_random (model:#GTree.model) row1 row2 =
    let name1 = model#get ~row: row1 ~column: UiPage1.random in
    let name2 = model#get ~row: row2 ~column: UiPage1.random in
    compare name1 name2 in

  UiPage1.store#set_sort_func 0 sort_by_nmb;
  UiPage1.store#set_sort_func 1 sort_by_random
